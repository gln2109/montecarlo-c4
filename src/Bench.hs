...
main = do
    ...
    forM_ testBoards (\(board, player) -> do evaluate (bestMoveParOld board player sims))
    end2 <- getCurrentTime
    putStrLn ("Basic Parallel Time: " ++ show (diffUTCTime end2 start2))

    start3 <- getCurrentTime
    forM_ testBoards (\(board, player) -> do evaluate (bestMovePar board player sims 625))
    end3 <- getCurrentTime
    putStrLn ("Chunked Parallel Time: " ++ show (diffUTCTime end3 start3))

    start4 <- getCurrentTime
    forM_ testBoards (\(board, player) -> do evaluate (bestMovePar board player sims 10))
    end4 <- getCurrentTime
